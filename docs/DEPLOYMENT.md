...
2. 输入名称：`it-toolbox`（或自定义名称）
3. 选择位置提示（建议选择自动）
4. 点击 **Create bucket**

### 3.4 第三步：连接 GitHub 创建 Pages 项目
...
### 3.5 第四步：在 Dashboard 绑定 KV、R2、AI 资源
...
1. 在同一页面找到 **R2 bucket bindings** 部分
2. 点击 **Add binding**
3. 填写：
   - **Variable name**: `FILES`
              ...
              └── Settings
                    └── Functions
                          └── R2 bucket bindings
                                └── Add binding
                                      ├── Variable name: FILES
                                      └── R2 bucket: 选择你的 R2 bucket
```

**截图参考位置**：
- 路径：Workers & Pages → 项目 → Settings → Functions → R2 bucket bindings
- 变量名必须为 `FILES`（与代码中的绑定名一致）
